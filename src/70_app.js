...
export class App {
  ...
  _bindInput() {
    ...
    const down = (x, y) => { drag = true; lx = x; ly = y; this.cam.idle = 0; this.cam.autoRot = false; this.takeOver(); };
    const move = (x, y) => {
      if (!drag) return;
      this.cam.az -= (x - lx) * 0.006;
      this.cam.el = clamp(this.cam.el + (y - ly) * 0.005, -0.5, 1.35);
  ...
  step(dtms) {
    ...
    c.cx = lerp(c.cx === undefined ? aimX : c.cx, aimX, k);
    c.cz = lerp(c.cz === undefined ? aimZ : c.cz, aimZ, k);
