...
export const SPECIES = {
  ...
  //   marginBias.ay 0.012  AND THIS IS THE KNOB THAT FIXED IT — a pure WIDTH
  //              knob, not a size knob. Over 0.16 -> 0.003 the margin's length
  //              is flat (x0.66 to x1.07, no trend) while its half-width falls
  //              17x. At 0.012 the margin grows aspect 0.040-0.058 over three
  //              seeds, inside a Norway spruce's 0.02-0.05, holding n50 = 1 —
  //              the single unbranched bundle a Picea needle has — on 78-87
  //              cells. Rendered at arm's length it is unmistakably a needle:
  //              one midvein, none of the reticulate scrawl a paddle carries.
  //
  //              0.012 RATHER THAN 0.008, and the difference is not cosmetic.
  //              Both sit in the spruce band and both give n50 = 1, so they are
  //              the same needle by every venation statistic — but 0.008 holds
  //              materially less crown (resolved fill 0.511 against 0.586 at
  //              equal organLen). Sit at the wide end of the band.
  //
  //              THE CELL COUNT IS THE REAL FLOOR ON THIS KNOB, not the aspect:
  //              55 cells at 0.005 and 44 at 0.003, where the blade is running
  //              out of tissue to canalise. `aspectFloor: 0.04` never bites and
  //              never did — the margin grows past it on its own.
  //
  ...
  //
  //   organLen 5.4  and this one is nearly free, which is what pays for the
  //              needle. On PADDLES it saturated at 3.0 (3.0 and 3.8 within
  //              0.003 of each other) and that entry was a statement about
  //              needles that already overlap. On needles it does not saturate:
  //              every step still buys, +0.051, +0.041, +0.033, +0.029 of
  //              resolved fill, at no extra organ. ROADMAP 13 item 0 predicted
  //              exactly this and it is why the change costs no organ budget.
  //
  //   organTilt 0.92  left alone, and MEASURED rather than assumed. Spreading
  //              the needles further off the axis is the other free axis and it
  //              is nearly dead: 0.92 -> 1.40 moves resolved fill by 0.009.
  //              Self-overlap is not what is limiting a needled crown.
  //
  // WHAT THE NEEDLE COST, AND WHAT PAID FOR IT (2026-07-31, ROADMAP 13 item 0).
  // Setting `ay` ALONE would have been worse than the Charlie Brown tree — 0.419
  // resolved fill against the pre-#32 specimen's 0.576 — because a needle 4.5x
  // narrower covers 4.5x less crown. It is affordable only because the crown is
  // heavily self-overlapping: over ay 0.16 -> 0.008 the blade TISSUE falls 4.85x
  // while the drawn ink falls 1.41x, and that gap is redundant foliage. Spending
  // `organLen` rather than organs recovers it: 0.618/0.567 resolved and
  // 0.725/0.712 on screen with `organBudget` untouched at 1200.
  //
  // ORGANS ARE THE WORST OF THE THREE LEVERS and that matters for ROADMAP 10b:
  // +50% of them buys +0.040 of resolved fill, where one step of `organLen`
  // buys +0.05 for free. `test/crown.mjs` is the instrument for all of this.
  //
  ...
  'Ashfall Spire': {
    ...
    mo: { R: 10, rCZ: 2.4, rPZ: 6.8, G: 0.0042 },
    sp: {
      elongation: 0.0034, organLen: 5.4, organTilt: 0.92, organRoll: 0.30,
      ...
      marginBias: { ay: 0.012, g1: 1.30, D: 0.70 },
    },
    pal: {
