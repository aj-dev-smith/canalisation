...
class Axis {
    ...
    if (this.floral) {
      ...
      if (wb) {
        ...
        const frac = org.whorl === 'sepal' ? (wb.sepalLen ?? 0.20)
          : org.whorl === 'petal' ? (wb.petalLen ?? 0.30) * pg
  ...
  elongate(dt, sp) {
    ...
    for (const org of this.organs) {
      // A RECEPTACLE does not elongate. A capitulum is a compressed axis whose
      // internodes never stretch — that is what makes it a head and not a
      // spike — so under `receptacle` every floret rides the TIP: the disc
      // stays one station while the axis elongating BELOW it becomes the
      // peduncle, which is how a real scape bolts. Without this the subapical
      // stretch strings 53 florets over 2.9 axis units (measured,
      // daisy_probe): a cylinder, not a disc. Default 0 skips: shipped exact.
      if (org.floral && (sp.receptacle || 0) > 0) { org.birthLen = this.length; continue; }
  ...
  updateRadii(sp) {
    ...
    for (const org of this.organs) {
      ...
      // THE RECEPTACLE — the dome, un-collapsed. A floral meristem is a disc
      // and q recorded where on it each organ was founded; placing them all on
      // the axis line throws that away. Offsetting the base by the founding
      // radius makes the flower the disc it grew as — rim organs at the rim,
      // the centre filled last, a capitulum. The offset eases in with dev the
      // way the tilt below does, so a founding is not a teleport. Default 0
      // skips it: shipped exactly.
      if (org.floral && (sp.receptacle || 0) > 0) {
        const rr = sp.receptacle * (1 - clamp(org.q || 0, 0, 1))
          * (0.25 + 0.75 * smoothstep(0.02, 0.5, org.dev || 0));
        v3addScaled(_zp, _zp, dir, rr);
      }
...
export const SPECIES_DEFAULTS = {
  ...
  receptacle: 0,     // the floral dome, un-collapsed. q RECORDS the radius each
                     // organ was founded at (1 - prim.r/floralR0); placing every
                     // floral organ on the axis line discards it. This is the
                     // rim radius in world units: an organ's base is offset
                     // along its own radial direction by receptacle*(1-q), so
                     // the flower becomes the disc the meristem actually was —
                     // a capitulum. 0 = collapsed to the axis, shipped exactly
  ...
  // attached above it, paste on a stump — each draining or arriving at polar-
  // transport speed (see `Axis.streamAt`). A CUT axis uses the stream whatever this
...
export class Plant {
  ...
  cut(ax, s) {
    ...
    ax.organs = kept;
    // a freed bud that left with the cutting is not a source on the stump any more;
    // what it had already sent down is in `level`, measured above
    if (ax.freeBuds) ax.freeBuds = ax.freeBuds.filter(o => o.birthLen <= s);
    const goneAxes = [];
