...
//
// So the objective is READABLE FRAME: cells holding something, minus cells whose
// contents are past full blur. It needs no weight between the two terms, because
// they are the same unit — thirds of a picture — and it is the quantity the shot
// is actually for. `ink` counts everything in front of the lens at any depth;
// `blur` is the subset inside the band.
const FL_BLUR_GX = 16, FL_BLUR_GY = 10;
const _blurCell = new Uint8Array(FL_BLUR_GX * FL_BLUR_GY);
const _inkCell = new Uint8Array(FL_BLUR_GX * FL_BLUR_GY);
const _frameOut = { blur: 0, ink: 0, read: 0 };
function flDirFrameLoad(F, ex, ey, ez, tx, ty, tz, kw, kh, g) {
  _frameOut.blur = 0; _frameOut.ink = 0; _frameOut.read = 0;
  ...
  _blurCell.fill(0); _inkCell.fill(0);
  for (const p of F.solid) {
    ...
    _inkCell[c] = 1;
    if (al <= zs) _blurCell[c] = 1;
  }
  for (let i = 0; i < _blurCell.length; i++) {
    _frameOut.blur += _blurCell[i];
    _frameOut.ink += _inkCell[i];
  }
  _frameOut.read = _frameOut.ink - _frameOut.blur;
  return _frameOut;
}
