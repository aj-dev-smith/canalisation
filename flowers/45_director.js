...
//
// ⚠ AND IT RETURNS THE INK AS WELL AS THE WALL, BECAUSE MINIMISING A WALL ALONE
// BUYS A HOLE. A solve that only asks "how little of the frame is unreadable"
// has its optimum at a bearing pointing out of the stand, and it will find one:
// on the film at ?garden=7&seed=21 the second dolly of the rotation came back
// with frame f160 EMPTY — fog, the HUD, and nothing else, for several seconds
// (shots/before21 vs shots/after21, same frame number, same step). The old shot
// was a wall; the first repair made a hole. tools/flowers_trans.mjs already
// carries this warning about the crane ("IS THERE ANYTHING IN THE PICTURE AT
// ALL — the other way a transition can be a bad two seconds, and the one a
// crane invents") and it applies here word for word.
//
// SO THE OBJECTIVE IS IN-FOCUS FRAME, AND BOTH OF ITS LIMITS ARE THE LENS'S.
// The first attempt at this scored `ink - blur`, ink being anything in front of
// the lens at any depth, and it did not close the hole: f160 came back empty a
// second time (shots/after21b) while the harness reported 19% ink, because a
// stand thirty units past the focal plane is ink to a projection and fog to a
// viewer. Anything nearer than uFocus - uRange is at full blur; anything FURTHER
// than uFocus + uRange is at full blur too, and this page then hazes it.
//
// A cell is therefore classified by its NEAREST surface, which is the only one a
// viewer sees, into exactly three states: WALL (nearer than the band), READ
// (inside it), SOFT (beyond it, or empty). `read` is what the solve maximises,
// and it is zero for a wall and zero for a hole without either being named — a
// bearing pointing out of the stand scores nothing because there is nothing at
// the distance this lens is focused on. Same rule the harness rasterises with.
const FL_BLUR_GX = 16, FL_BLUR_GY = 10;
const _cellZ = new Float64Array(FL_BLUR_GX * FL_BLUR_GY);
const _frameOut = { blur: 0, read: 0, ink: 0 };
function flDirFrameLoad(F, ex, ey, ez, tx, ty, tz, kw, kh, g) {
  _frameOut.blur = 0; _frameOut.read = 0; _frameOut.ink = 0;
  const dx = tx - ex, dy = ty - ey, dz = tz - ez;
  const L = Math.hypot(dx, dy, dz);
  if (!(L > 1)) return _frameOut;
  const ux = dx / L, uy = dy / L, uz = dz / L;
  const rng = Math.max(FL_DIR_DOLLY_DOF.min, L * FL_DIR_DOLLY_DOF.k);
  const zs = L - rng, zf = L + rng;
  ...
  for (const p of F.solid) {
    ...
    const c = j * FL_BLUR_GX + i;
    if (al < _cellZ[c]) _cellZ[c] = al;
  }
  for (let i = 0; i < _cellZ.length; i++) {
    const z = _cellZ[i];
    if (z > 1e17) continue;
    _frameOut.ink++;
    if (z < zs) _frameOut.blur++;
    else if (z <= zf) _frameOut.read++;
  }
  return _frameOut;
}
...
//
// WHAT IT COSTS, measured rather than waved at: 27 candidates x 4 positions of
// the travel x 11,638 points of F.solid is 5.10 ms, once per dolly cut — ten
// times flDirArc's 0.51 ms and about a fifteenth of what a grown stand of seven
// already spends capturing a frame. It lands once every ~127 s, on a frame that
// is already the most expensive in the rotation because it is also solving the
// crane. If the candidate grid ever grows, that is the number to re-take.
// WHERE ALONG ITS OWN LINE THE CHORD SITS, in units of R. This is the traverse's
// `t0` window placed on the mass, made discrete: the chord is 1.9 R long and
// sliding it by +-0.45 R moves which half of the stand the camera is abreast of
// without changing the standoff, the travel or the eye height — so the shot's
// construction is untouched and only its framing moves.
//
// IT WAS A/B'D RATHER THAN ASSUMED, because it is the one term here that changes
// the track's geometry (with an offset the eye's distance from the field's middle
// runs 41.5 -> 57.4 rather than 46.8 -> 49.0). Blur wall over the dwell, median
// over eight bearings, tools/flowers_hold.mjs:
//     heading search only            seed 21  10%      seed 1337  21%, 3/104 wall
//     heading + this offset          seed 21   5%      seed 1337  19%, 0/104 wall
// The offset is what removes the last literal walls on 1337 and takes that seed's
// peak from 41% to 33%. [0] restores the heading-only behaviour exactly.
...
function flDirDollySolve(dir, F, bb, D) {
  ...
  for (let i = -4; i <= 4; i++) {
    ...
    for (const o of FL_DOLLY_OFFS) {
      const off = o * F.R;
      // the score is READABLE FRAME summed over the chord, and it is MAXIMISED;
      // `load` is its negative so the comparison below reads as a cost.
      let load = 0;
      for (let k = 0; k < 4; k++) {
        const t = off + (2 * (k / 3) - 1) * F.R * 0.95;
        load -= flDirFrameLoad(F, F.cx + c * D - s * t, ey, F.cz + s * D + c * t,
          tx, ty, tz, kw, kh, g).read;
      }
