...
//     low     9   punctuation
// 71 s of hold + 5 transitions = 96 s, against 92 s for the old four.
const FL_DIR_SHOTS = [
...
// smoothstep peaks at 1.5x its mean). The establishing shot stands at 81.8
// units where `bank` stands at 47.9, which takes that move to ~130 units and
// would have peaked near 39 u/s — 2.4 m/s in WORLD.unitM, a jog. 7.5 s puts it
// back at 26, exactly the peak the other four already have. It is NOT the
// 59-62 u/s whip the close-up's arrival gate was built for; it is the same
...
// HEIGHT binds (58.0 against 33.6); at ?garden=12 it is 106.8 wide and 44.8
// tall and the WIDTH binds (117.1 against 76.4). ?garden=7 is height-bound at
// 81.8 against 67.1. A pose that solved only one of them is inside the field
...
// something. The numbers: fW 0.86, fH 0.78, feet at 0.16, horizon at 0.46.
// ?estab=fW,fH,bF,hz sweeps all four; the ladder that chose them is in the
// branch notes.
//
// ⚠ THIS IS NOT PHASE 1'S FRAMING OF THE BOUND, though the distances are
// comparable (81.8 here against 84.2 there). That one framed a SPHERE from
// above and put six plants in an overlapping column; this one stands off the
// short axis at 1.15 m of eye height and looks across, so the long axis lies
// across the frame and the depression angle is 2 degrees. Far away and above
// is a diagram; far away and level is a landscape.
const FL_ESTAB = [0.86, 0.78, 0.16, 0.46];
