...
// Placement is plantGarden's jittered ring with one addition: a minimum
// spacing, rejection-sampled deterministically, so corollas don't
// interpenetrate. startAt staggers germination over ~0..FL_GARDEN_STAGGER
// world steps — the piece is "growing a garden", so the growth IS the show.
// Everything is drawn from one mulberry32 stream off the base seed: the URL
// fully decides the field, same discipline as everything else here.
...
function flGardenPlan(baseSeed, n, opts = {}) {
  ...
  for (let i = 0; i < n; i++) {
    let origin = [0, 0, 0];
    if (i > 0) {
      // plantGarden's jittered ring, with the hero's slot (the centre) left
      // out and a minimum spacing enforced by deterministic rejection: 24
      // tries against every already-placed origin, keeping the last try if
      // the clearing is genuinely too crowded (n=12 at radius 7 never is —
      // measured: 0 rejections exhausted over seeds 1..64).
      for (let t = 0; t < 24; t++) {
        const a = (i - 0.5) / Math.max(1, n - 1) * TAU + (r() - 0.5) * 0.9;
        const d = rad * (0.55 + 0.75 * r());
        origin = [Math.cos(a) * d, 0, Math.sin(a) * d];
        let clear = true;
        for (const p of plan) {
          if (Math.hypot(origin[0] - p.origin[0], origin[2] - p.origin[2]) < FL_GARDEN_SPACING) {
            clear = false; break;
          }
        }
        if (clear) break;
    ...
    plan.push({
      ...
      seed: i === 0 ? baseSeed : (baseSeed + i * 7919) >>> 0,   // plantGarden's stride
      origin,
      startAt: i === 0 ? 0 : Math.floor(r() * FL_GARDEN_STAGGER),
    });
  }
  return plan;
}
