...
// nearly 5x in the direction that shows: at garden=7 seed=21 two specimens had
// grown THROUGH each other. Grown headlessly at 3000 steps, taking horizontal
// reach hypot(x,z) off the DRAWN streams — the same argument as floralBounds,
// measure what is on screen rather than a guess reconstructed from organ
// lengths. `maxR` is the outermost vertex, `r90` the radius nine tenths of the
// drawn geometry lives inside: the BODY, as against one flopping arm.
// Units; WORLD.unitM = 0.0625 m/unit.
//
// 8 flowering species x 4 forms x 5 seeds (the seeds a garden actually deals,
// baseSeed + i*7919), 160 cells:
//   maxR   p10  3.6   med  9.7   p75 14.4   p90 23.2   max 70.9
//   r90    p10  2.2   med  5.0   p75  7.5   p90 12.1   max 20.4
//
// ⚠ TWO THINGS THAT SWEEP FOUND AND THE FIRST, ONE-SEED VERSION OF IT DID NOT.
// The one-seed table (med maxR 9.49, med r90 5.76) got the MIDDLE right and the
// TAIL completely wrong, and the tail is what collides:
//   - reach is a statement about a SEED as much as a species. Ember Creeper
//     columbine reads maxR 7.4 at seed 21 and 65.9 at seed 31697 — the same
//     species and the same form, 9x apart.
//   - reach is not bounded in TIME for that species. Sampled every 200 steps,
//     seed 31697 goes 11.9 (step 1000) -> 28.8 -> 44.4 -> 70.9 (step 3200) and
//     is still climbing linearly: a creeper whose axes never arrest. NO fixed
//     spacing keeps a creeper off its neighbours, and none should be expected
//     to — that is the organism's business (organBudget / apical control), not
//     placement's, and it is why the numbers below are quoted as medians.
//
// ⚠ SO 12 IS A STATED FRACTION AND IT SHOULD BE READ AS ONE. It is full
// clearance of two median BODIES (2 x 5.0 = 10.0) with margin, ~80% of a p75
// pair (15.0), and 62% of full clearance of two median ARMS (2 x 9.7 = 19.4).
// Full arm clearance was measured and rejected by looking: at 19-44 units apart
// plants 20-48 units tall stop being a stand and read as a row of isolated
// specimens. Two neighbours' outermost peduncles may cross; their bodies do
// not, which is what a meadow looks like and what the defect actually was.
// Grown and checked end to end (garden=7 seed=21 at step 3600): 3 of 21 pairs
// overlap at r90 and all three involve the runaway creeper or the hero.
