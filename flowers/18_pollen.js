...
// so a field's per-plant plume is thinner than a solo page's. Measured
// (test/flowers_pollen.mjs, garden=7 seed=21): the field's anther count climbs
// 12 -> 59 -> 96 over steps 1500-4500 against the hero's own 16, and the
// population runs 5-31% of the cap over that whole range. The cap does not
// bind at seven, so nothing is being traded away yet; at the 12 the URL allows
// it would begin to, and the honest fix then is the cap and not the rate,
// because the rate is a per-anther statement about a flower and the cap is a
// statement about the frame. 7 floats a grain: 4096 grains is 114 kB.
//
// ------------------------------------------------------------- TWO CLOCKS
//
...
const FL_POLLEN = {
  ...
  // HOW FAR PAST THE OUTERMOST PLANT A GRAIN IS STILL PART OF THE PICTURE,
  // in world units beyond the clearing's rim. This is a LEGIBILITY number and
  // not physics — a real grain keeps going — and it is needed because the
  // point pass has no fog and floors gl_PointSize at 1 px (30_scene.js), so a
  // grain does not fade or shrink out of the frame at any distance: at 90
  // units it is still a full-brightness one-pixel dot on the sky. Measured on
  // the shipped 90: the mean grain sat 30-46 units from the clearing's centre
  // and 60-85% of the whole population was outside the field the plants stand
  // in, so most of the cap was spent on sky. One plant's room (the measured
  // FL_GARDEN_SPACING) past the last plant keeps the plume in among the
  // flowers where the light is. ?pol=rate,beyond sweeps both.
  beyond: 12,
};
...
class FlPollen {
  ...
  constructor(seed, keyCol, plan) {
    ...
    if (typeof location !== 'undefined') {
      ...
      if (s.length === 2 && s.every(isFinite)) { rate = s[0]; beyond = s[1]; }
    }
    this.rate = rate;
    // the clearing's rim: where the outermost plant stands. Solo pages have no
    // plan and no rim, so the story is one plant's room wide either way.
    let rim = 0;
