...
const FL_POLLEN = {
  ...
  // light is and spends nothing. ?pol=rate,beyond sweeps both.
  //
  // It also settles the ground: at 90, 48% of grains ended by touching y=0 —
  // all of them far downwind, drawn at the 1-px floor, invisible — and at 12
  // none do. Stokes settling is 3.25 cm/s against a wind that moves a grain
...
class FlPollen {
  ...
  // per-frame cap — so THE WATCHED PAGE WAS ALWAYS FINE AND ?ff= WAS NOT: at
  // its 40-step chunk a grain jumps 12.8 units per call, further than the
  // shortest gust is long, and can be emitted and killed inside one call. A
  // fast-forwarded screenshot was showing a population that never existed.
  _substep(wf) {
